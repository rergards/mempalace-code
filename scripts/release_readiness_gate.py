...
CUSTOM_MODEL_TORCH_VERSION = "2.13.0"
CUSTOM_MODEL_TORCH_CPU_INDEX = "https://download.pytorch.org/whl/cpu"
EXPECTED_CUSTOM_MODELS_INSTALL_COMMAND = (
    "python -m pip install --force-reinstall --no-cache-dir "
    f"torch=={CUSTOM_MODEL_TORCH_VERSION} --index-url {CUSTOM_MODEL_TORCH_CPU_INDEX} && "
...
"""
INSTALLED_CUSTOM_MODEL_ACTION_PROBE = (
    "from mempalace_code.storage import CUSTOM_MODELS_INSTALL_COMMAND; "
    "print(CUSTOM_MODELS_INSTALL_COMMAND)"
)
INSTALLED_CUSTOM_MODEL_CPU_PROBE = r"""import importlib.metadata
import json
import re
import torch
...
INSTALLED_GOLDEN_FORBIDDEN_OUTPUT_MARKERS = (
    ("python-traceback", "Traceback (most recent call last)"),
    ("version-check-notice", "Enable periodic new-version checks"),
    ("new-version-notice", "New version available"),
    ("token-not-saved", "The token has not been saved"),
    ("token-settings-url", "hf.co/settings/tokens"),
    ("buffered-stdout-noise", "fake buffered stdout noise"),
    ("buffered-stderr-noise", "fake buffered stderr noise"),
    ("fd-stdout-noise", "fake fd stdout noise"),
    ("fd-stderr-noise", "fake fd stderr noise"),
)
INSTALLED_GOLDEN_FORBIDDEN_OUTPUT = tuple(
    marker for _safe_id, marker in INSTALLED_GOLDEN_FORBIDDEN_OUTPUT_MARKERS
)
...
def _run_installed_non_regular_source_scenario(
    ...
    *,
    ...
    def forbidden_detail(output: str, stage: str, command_class: str) -> str | None:
        for safe_id, marker in INSTALLED_GOLDEN_FORBIDDEN_OUTPUT_MARKERS:
            if marker in output:
                return (
                    "forbidden subprocess output detected; "
                    f"marker={safe_id}; stage={stage}; command_class={command_class}"
                )
        return None

    def run(args: list[str], *, stage: str, command_class: str):
        result = _run_installed_cli(
            run_subprocess,
            command_prefix,
            args,
        ...
        output = (result.stdout or "") + (result.stderr or "")
        forbidden = forbidden_detail(output, stage, command_class)
    ...
    def take_output(item: object, output: list[str]) -> str:
        ...
        forbidden = forbidden_detail(item, "watcher-stream", "watch")
        if forbidden is not None:
            raise RuntimeError(forbidden)
        output.append(item)
        return item
    ...
    try:
        ...
        require_ok(init, "init", "Config saved:")
        mine = run(
            ["--palace", str(project_palace), "mine", str(project)],
            stage="project-mine",
            command_class="mine",
        )
        ...
        for kind, term in legacy_terms.items():
            stale = run(
                ["--palace", str(project_palace), "search", term, "--results", "10"],
                stage="stale-search",
                command_class="search",
            )
        ...
        mine_all = run(
            ["--palace", str(project_palace), "mine-all", str(scenario_root)],
            stage="mine-all",
            command_class="mine-all",
        )
        ...
        forbidden = forbidden_detail(watcher_summary, "watcher-stream", "watch")
        if forbidden is not None:
            raise RuntimeError(forbidden)
...
def _custom_model_cpu_evidence_error(payload: object) -> str | None:
    """Require the pinned CPU Torch runtime and reject CUDA-family packages."""
    if not isinstance(payload, dict) or set(payload) != {"version", "cuda", "distributions"}:
        return "installed custom-model Torch evidence has an invalid shape"
    version = payload["version"]
    if not isinstance(version, str) or version.partition("+")[0] != CUSTOM_MODEL_TORCH_VERSION:
        return "installed custom-model Torch version mismatched the CPU pin"
    if payload["cuda"] is not None:
        return "installed custom-model Torch runtime exposes CUDA"
    distributions = payload["distributions"]
    if (
        not isinstance(distributions, list)
        or "torch" not in distributions
        or any(
            not isinstance(name, str) or name != re.sub(r"[-_.]+", "-", name).lower()
            for name in distributions
        )
    ):
        return "installed custom-model distribution evidence is malformed"
    forbidden = sorted(
        name
        for name in distributions
    ...
    if forbidden:
        return f"installed custom-model contour contains forbidden distribution: {forbidden[0]}"
    return None
...
def _run_installed_extra_and_export_reconciliation(
    *,
    ...
    def install_extra(extra: str) -> tuple[Path, dict[str, str], Path, Path]:
        ...
        if extra == "custom-models":
            torch_install = completed(
                [
                    str(python_bin),
                    "-m",
                    "pip",
                    ...
                    "--force-reinstall",
                    "--no-cache-dir",
                    f"torch=={CUSTOM_MODEL_TORCH_VERSION}",
                    "--index-url",
                    CUSTOM_MODEL_TORCH_CPU_INDEX,
                ],
                env=setup_env,
                timeout=INSTALLED_GOLDEN_TIMEOUT,
            )
            if torch_install.returncode != 0:
                raise RuntimeError("custom-models CPU Torch installation failed")
        install = completed(
    ...
    try:
        ...
        action = completed(
            [str(base_python), "-c", INSTALLED_CUSTOM_MODEL_ACTION_PROBE], env=base_env
        )
        if (
            action.returncode != 0
            or not _installed_output_is_clean(action)
            or action.stderr != ""
            or action.stdout != f"{EXPECTED_CUSTOM_MODELS_INSTALL_COMMAND}\n"
        ):
            raise RuntimeError("candidate custom-model recovery action mismatched release oracle")
        custom_python, custom_env, custom_attempts, custom_marker = install_extra("custom-models")
        custom_cpu = json_probe(
            custom_python,
            INSTALLED_CUSTOM_MODEL_CPU_PROBE,
            [],
            custom_env,
            "installed custom-model CPU Torch probe",
        )
        custom_cpu_error = _custom_model_cpu_evidence_error(custom_cpu)
        if custom_cpu_error:
            raise RuntimeError(custom_cpu_error)
        custom = json_probe(
            custom_python,
