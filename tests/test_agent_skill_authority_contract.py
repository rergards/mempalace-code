...
PLAN_BASELINE_COMMIT = "e9941093d22ac158d0c391267d04787f098c4405"
...
def test_plan_lifecycle_bulk_edit_preserves_prechange_content():
    baseline_paths = set(
        _git("ls-tree", "-r", "--name-only", PLAN_BASELINE_COMMIT, "docs/plans").splitlines()
    )
    current_paths = {
        str(path.relative_to(ROOT)) for path in PLANS.glob("*.md") if path != PLAN_CONTRACT
    }
    assert current_paths == baseline_paths

    for relative in sorted(baseline_paths):
        baseline = _git("show", f"{PLAN_BASELINE_COMMIT}:{relative}")
        current = (ROOT / relative).read_text(encoding="utf-8")
        if current == baseline:
            continue
        metadata, _, _ = _front_matter(baseline)
        if (
            metadata.get("status") in PLAN_LIFECYCLE_STATES
            and metadata.get("authority") == "non_authoritative"
        ):
            _, _, baseline_body = _front_matter(baseline)
            current_metadata, _, current_body = _front_matter(current)
            assert current_body == baseline_body, relative
            assert current_metadata.get("status") in PLAN_LIFECYCLE_STATES, relative
            assert current_metadata.get("authority") == "non_authoritative", relative
            continue
        lines = baseline.splitlines(keepends=True)
        slug_index = next(index for index, line in enumerate(lines) if line.startswith("slug:"))
        inserted = [
            f"status: {_expected_plan_status(ROOT, str(metadata['slug']))}\n",
            "authority: non_authoritative\n",
        ]
        expected = "".join(lines[: slug_index + 1] + inserted + lines[slug_index + 1 :])
