...
def _run_custom_model_contour(
    tmp_path: Path,
    *,
    action: str | None = None,
    cpu_payload: object | None = None,
    failed_install: str | None = None,
):
    commands: list[list[str]] = []
    behavior_probes: list[str] = []
    action = action if action is not None else rrg.EXPECTED_CUSTOM_MODELS_INSTALL_COMMAND
    cpu_payload = cpu_payload or {
        "version": "2.13.0+cpu",
        "cuda": None,
        "distributions": ["pip", "sentence-transformers", "torch"],
    }
    site_script = "import site; print('site')"
    smoke = SimpleNamespace(
        _SITE_PACKAGES_SCRIPT=site_script,
        _SITE_GUARD="from pathlib import Path\n",
        _SITE_GUARD_PTH="mempalace-socket-guard.pth",
        probe_candidate_extra_metadata=lambda *args, **kwargs: SimpleNamespace(
            ok=True,
            runtime_extras=("custom-models", "spellcheck", "treesitter", "watch"),
            detail="",
        ),
    )
    ...
    def completed(command, **kwargs):
        ...
        marker = env.get("MEMPALACE_SOCKET_GUARD_LOADED")
        if marker:
            Path(marker).touch()
        if rendered[1:3] == ["-m", "venv"]:
            (Path(rendered[3]) / "bin").mkdir(parents=True)
            return SimpleNamespace(returncode=0, stdout="", stderr="")
        if rendered[1:4] == ["-m", "pip", "install"]:
            is_cpu = any(value == "torch==2.13.0" for value in rendered)
            is_custom_extra = rendered[-1].endswith("[custom-models]")
            should_fail = (failed_install == "cpu" and is_cpu) or (
                failed_install == "extra" and is_custom_extra
            )
            return SimpleNamespace(returncode=1 if should_fail else 0, stdout="", stderr="")
        if len(rendered) >= 3 and rendered[1] == "-c":
            code = rendered[2]
            if code == site_script:
                site_dir = Path(rendered[0]).parent.parent / "site-packages"
                site_dir.mkdir(parents=True)
                return SimpleNamespace(returncode=0, stdout=json.dumps([str(site_dir)]), stderr="")
            if code == rrg.INSTALLED_EXPORT_PROBE:
                payload = {}
            elif code == rrg.INSTALLED_CUSTOM_MODEL_ABSENT_PROBE:
                payload = {
                    "dependency_absent": True,
                    "preflight_failed": True,
                    "palace_absent": True,
                }
            elif code == rrg.INSTALLED_CUSTOM_MODEL_ACTION_PROBE:
                return SimpleNamespace(returncode=0, stdout=f"{action}\n", stderr="")
            elif code == rrg.INSTALLED_CUSTOM_MODEL_CPU_PROBE:
                payload = cpu_payload
            elif code == rrg.INSTALLED_CUSTOM_MODEL_PROBE:
                behavior_probes.append("custom-model")
                payload = {"dimensions": 3, "rows": 2, "normalized": True}
            elif code == rrg.INSTALLED_SPELLCHECK_PROBE:
                installed = "extra-spellcheck" in rendered[0]
                payload = {
                    "autocorrect": installed,
                    "output": (
                        "> misspelled quick brown fox\nassistant: unchanged\n"
                        if installed
                        else "> mispelled quick brown fox\nassistant: unchanged\n"
                    ),
                }
            elif code == rrg.INSTALLED_TREESITTER_PROBE:
                payload = {}
            elif code == rrg.INSTALLED_MIGRATION_PROBE:
                payload = {
                    "returncode": 1,
                    "stderr_exact": True,
                    "bridge_modules_absent": True,
                    "chromadb_dependency_absent": True,
                }
                return SimpleNamespace(
                    returncode=0,
                    stdout=(
                        rrg.INSTALLED_MIGRATION_EVIDENCE_MARKER
                ...
                raise AssertionError(f"unexpected probe: {code[:80]}")
            return SimpleNamespace(returncode=0, stdout=json.dumps(payload), stderr="")
        if Path(rendered[0]).name == "mempalace-code-alias":
            alias = Path(rendered[-1]) / "mempalace"
            alias.symlink_to("mempalace-code")
            return SimpleNamespace(returncode=0, stdout="", stderr="")
        raise AssertionError(f"unexpected command: {rendered}")

    exports = rrg.INSTALLED_PUBLIC_EXPORTS
    export_receipts = {
        f"export:{owner}:{member}": True
        for owner, members in exports.items()
        for member in members
        if member != "main_alias"
    }
    ...
    with (
        patch.object(rrg, "_parse_installed_public_exports", return_value=exports),
        patch.object(rrg, "_installed_treesitter_evidence_error", return_value=None),
    ):
        result = rrg._run_installed_extra_and_export_reconciliation(
            root=tmp_path / "repo",
            wheel=tmp_path / "candidate.whl",
            expected_version="1.2.3",
            temp_root=tmp_path / "contours",
            base_venv=base_venv,
            base_python=base_venv / "bin" / "python",
            base_env={"PATH": "/usr/bin"},
            setup_env={"PATH": "/usr/bin"},
            neutral_cwd=neutral_cwd,
            hf_home=tmp_path / "hf",
            watch_receipt=True,
            export_receipts=export_receipts,
            smoke=smoke,
            run_subprocess=completed,
        )
    return result, commands, behavior_probes


def test_custom_model_cpu_install_contract(tmp_path):
    help_result = subprocess.run(
        ["/usr/bin/python3", str(ROOT / "scripts" / "release_readiness_gate.py"), "--help"],
        capture_output=True,
        text=True,
        check=False,
    )
    assert help_result.returncode == 0
    assert rrg.EXPECTED_CUSTOM_MODELS_INSTALL_COMMAND == storage_owner.CUSTOM_MODELS_INSTALL_COMMAND

    result, commands, behavior_probes = _run_custom_model_contour(tmp_path)

    assert result is None
    cpu_index = next(index for index, command in enumerate(commands) if "torch==2.13.0" in command)
    extra_index = next(
        index for index, command in enumerate(commands) if command[-1].endswith("[custom-models]")
    )
    assert cpu_index < extra_index
    assert commands[cpu_index][4:] == [
        "--force-reinstall",
        "--no-cache-dir",
        "torch==2.13.0",
        "--index-url",
    ...
    assert all("--extra-index-url" not in command for command in commands)
    assert behavior_probes == ["custom-model"]


@pytest.mark.parametrize(
    ("case", "action", "cpu_payload", "failed_install", "expected"),
    [
        ("oracle", "python -m pip install wrong", None, None, "mismatched release oracle"),
        ("cpu-install", None, None, "cpu", "CPU Torch installation failed"),
        ("extra-install", None, None, "extra", "custom-models contour installation failed"),
        (
            "version",
            None,
            {"version": "2.12.0+cpu", "cuda": None, "distributions": ["torch"]},
            None,
            "version mismatched",
        ),
        (
            "cuda-runtime",
            None,
            {"version": "2.13.0+cu130", "cuda": "13.0", "distributions": ["torch"]},
            None,
            "runtime exposes CUDA",
        ),
        (
            "triton",
            None,
            {"version": "2.13.0+cpu", "cuda": None, "distributions": ["torch", "triton"]},
            None,
            "forbidden distribution: triton",
        ),
        (
            "cuda-package",
            None,
            {"version": "2.13.0+cpu", "cuda": None, "distributions": ["cuda-runtime", "torch"]},
            None,
            "forbidden distribution: cuda-runtime",
        ),
        (
            "nvidia-package",
            None,
            {"version": "2.13.0+cpu", "cuda": None, "distributions": ["nvidia-cublas", "torch"]},
            None,
            "forbidden distribution: nvidia-cublas",
        ),
    ],
)
def test_custom_model_cpu_install_failures(
    tmp_path, case, action, cpu_payload, failed_install, expected
):
    result, commands, behavior_probes = _run_custom_model_contour(
        tmp_path,
        action=action,
    ...
    if case == "oracle":
        assert not any("torch==2.13.0" in command for command in commands)
        assert not any(command[-1].endswith("[custom-models]") for command in commands)
    if case in {"version", "cuda-runtime", "triton", "cuda-package", "nvidia-package"}:
        assert behavior_probes == []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "missing-diagnostic",
        *[
            ("forbidden-subprocess", channel, safe_id, marker)
            for safe_id, marker in rrg.INSTALLED_GOLDEN_FORBIDDEN_OUTPUT_MARKERS
            for channel in ("stdout", "stderr")
        ],
        (
            "forbidden-watcher",
            "stream",
            rrg.INSTALLED_GOLDEN_FORBIDDEN_OUTPUT_MARKERS[0][0],
            rrg.INSTALLED_GOLDEN_FORBIDDEN_OUTPUT_MARKERS[0][1],
        ),
        "network-attempt",
        "repository-drift",
        "neutral-drift",
...
def test_installed_non_regular_source_scenario(
    tmp_path: Path, fault: str | tuple[str, str, str, str]
):
    forbidden_case = fault if isinstance(fault, tuple) else None
    fault = forbidden_case[0] if forbidden_case is not None else fault
    repository_root = tmp_path / "repository"
    repository_root.mkdir()
    (repository_root / "tracked.txt").write_text("stable\n", encoding="utf-8")
    ...
    def run(command, **kwargs):
        ...
        args = command[1:]
        if args[0] == "init":
            if fault == "forbidden-subprocess":
                assert forbidden_case is not None
                _kind, channel, _safe_id, marker = forbidden_case
                hostile = (
                    f"{marker}\nraw-output-must-not-leak\n"
                    f"token=credential-shaped-value\n{tmp_path}\n"
                )
                return SimpleNamespace(
                    returncode=0,
                    stdout="Config saved:\n" + (hostile if channel == "stdout" else ""),
                    stderr=hostile if channel == "stderr" else "",
                )
    ...
    class Watcher:
        def __init__(self, command, **kwargs):
            ...
            if fault == "forbidden-watcher":
                assert forbidden_case is not None
                _kind, _channel, _safe_id, marker = forbidden_case
                summary.extend(
                    [
                        f"{marker}\n",
                        "raw-watcher-output-must-not-leak\n",
                        f"token=credential-shaped-value\n{tmp_path}\n",
                    ]
                )
    ...
    if fault == "success":
        ...
        expected_marker = {
            ...
            "forbidden-subprocess": "forbidden subprocess output detected",
            "forbidden-watcher": "forbidden subprocess output detected",
        ...
        if forbidden_case is not None:
            _kind, _channel, safe_id, marker = forbidden_case
            expected_stage = "project-init" if fault == "forbidden-subprocess" else "watcher-stream"
            expected_class = "init" if fault == "forbidden-subprocess" else "watch"
            assert f"marker={safe_id}" in row["detail"]
            assert f"stage={expected_stage}" in row["detail"]
            assert f"command_class={expected_class}" in row["detail"]
            assert marker not in row["detail"]
            assert "raw-output-must-not-leak" not in row["detail"]
            assert "raw-watcher-output-must-not-leak" not in row["detail"]
            assert "credential-shaped-value" not in row["detail"]
            assert len(row["detail"]) <= 1200
            assert row["detail"].count(f"rerun: {rrg.INSTALLED_GOLDEN_COMMAND}") == 1
        if fault == "home-cache-drift":
