...
class TestSearchCompactMode:
    @pytest.mark.parametrize("argv, expected", [([], False), (["--compact"], True)])
    def test_compact_selection_is_passed_to_search(self, tmp_path, capsys, argv, expected):
        palace = str(tmp_path / "palace")
        ...
        with patch("mempalace_code.searcher.search") as mock_search:
            with patch.object(
                sys,
                "argv",
                ["mempalace-code", "--palace", palace, "search", "query", *argv],
            ):
                main()
        ...
        assert captured.out == ""
        assert captured.err == ""
        assert mock_search.call_args.kwargs["compact"] is expected

    def test_compact_preserves_results_boundary(self, tmp_path, capsys):
        palace = str(tmp_path / "palace")
