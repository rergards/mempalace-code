...
- **Custom-model boundary**: arbitrary Hugging Face names and local
  SentenceTransformer paths require `python -m pip install --force-reinstall --no-cache-dir torch==2.13.0 --index-url https://download.pytorch.org/whl/cpu && python -m pip install 'mempalace-code[custom-models]'`.
