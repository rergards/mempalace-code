...
task_contract:
  ...
  verification:
    ...
    - id: VER-2
      command: "rg -q 'mempalace-watch-.*service' docs/UPDATES.md docs/AGENT_INSTALL.md && rg -q 'disabled until.*install' docs/UPDATES.md && rg -q 'disabled until.*install' docs/AGENT_INSTALL.md"
...
- Complete discovery before probing active state, then cache that selected-unit result on the adapter for the transaction. This keeps the recorded `watcher_unit`, stop/start calls, and post-restart validation bound to one systemd-user unit even if the unit list changes mid-update.
