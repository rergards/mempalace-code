...
- Do not edit `.github/workflows/ci.yml`: its test, lint, and package jobs already invoke the owning test/guard/preflight surfaces. Hosted completion at the exact implementation SHA is required release evidence; local commands prove behavior and workflow prerequisites but do not claim an unobserved hosted result.
