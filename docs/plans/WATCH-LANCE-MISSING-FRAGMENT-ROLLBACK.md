...
- Treat a pre-run backup as required only when `<palace>/lance` exists and contains data. Use a non-mutating check for this boundary; do not create a Lance table, migrate schema, or load embeddings just to decide whether the startup backup is needed. A first-ever watch has nothing to preserve, so it should not be blocked by backup creation.
...
- Keep tests mostly mocked in `tests/test_watcher.py` so they prove ordering and exit behavior without running real watchfiles loops or embedding-heavy mining. Patch backup, mine, watch, open_store, and recovery calls directly so failures are deterministic; use existing Lance health/storage tests as regressions rather than duplicating Lance corruption setup in watcher tests.
