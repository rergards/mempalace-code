...
For the custom-model contour, the candidate wheel must advertise this exact
retry-safe recovery action before the extra contour is created:
This pip action must never use `--force` or `--extra-index-url`; `--force-reinstall`
is the bounded retry flag.
...
The contour runs the pinned CPU-index Torch step before installing the exact
candidate wheel with `[custom-models]`. Qualification then requires Torch release
2.13.0, no CUDA runtime, and no `triton`, `cuda-*`, or `nvidia-*` distribution
before the local SentenceTransformer behavior probe. Any mismatch fails the
existing aggregate row before later custom-model behavior runs.
