...
git grep -n -o -E 'docs/[a-zA-Z0-9_./-]+\.md' -- docs AGENTS.md CLAUDE.md .claude/ \
  ...
  ':(exclude)docs/UPSTREAM_COMPARISON.md' \
  ':(exclude)docs/quality/upstream-comparison.json' \
  ':(exclude).claude/prompts/**' | while IFS=: read -r file line ref_path; do
  [ ! -f "$ref_path" ] && echo "DEAD REF: $file:$line -> $ref_path"
done
```
