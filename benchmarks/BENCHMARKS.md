...
**The results are auditable.** The benchmark scripts write full result JSONL/JSON files when run — every question, every retrieved document, every score. Historical raw result files from the original benchmark runs are not committed to this repository; regenerate them using the commands in [Reproducing Every Result](#reproducing-every-result).
...
Historical raw result files from the original benchmark runs are **not committed** to this repository. The table below lists the files produced during those runs; regenerate them using the commands in [Reproducing Every Result](#reproducing-every-result).
...
| `results_locomo_hybrid_session_top10_<timestamp>.json` | locomo hybrid_v5 | 88.9% R@10 | Honest — top-10, no rerank |
...
Beats Memori (81.95%) by 7pp with no reranking. Result file: `results_locomo_hybrid_session_top10_<timestamp>.json`
